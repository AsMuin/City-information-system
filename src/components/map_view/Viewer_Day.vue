<template>
    ...
    <router-link to="/city_Night">
        <el-button id="btn" size="large" type="danger" text>夜间模式</el-button>
    </router-link>
...
async function loadTiles(): Promise<void> {
    await Cesium.Cesium3DTileset.fromIonAssetId(75343)
        // await Cesium.Cesium3DTileset.fromUrl('http://localhost:8090/iserver/services/3D-ThreeDTilesCache-tileset/rest/realspace/datas/tileset/config')
        .then(tileset => {
...
const STREET = {
    ...
    load: async function loadStreet() {
        ElMessage({message: '加载路网中...', type: 'info', duration: 0});
        const res = await Geoserver_WFS('NewYork', 'NewYork_Road');
        console.log(res);
        let features = [];
        if (res.data.features && res.data.features.length > 0) {
            ...
            (this.StreetPolyLine as any) = addStreet(
                viewer,
                features,
                '/PolyLine_material/Snipaste5.png'
            );
        }
