<template>
    <el-dialog v-model="centerDialogVisible" title="修改参数" width="500" :modal=false center>
        <div class="slider-demo-block">
            ...
            <el-slider v-model="Heat_opacity" :min=0.1 :max=1 :step=0.01 @input='updateHeat_opacity()' />
        </div>
        <div class="slider-demo-block">
            <span class="demonstration">变化程度</span>
            <el-slider v-model="Heat_blur" :min=0.1 :max=1 :step=0.01 @input='updateHeat_blur()' />
        </div>
        <div class="slider-demo-block">
            <span class="demonstration">半径</span>
            <el-slider v-model="Heat_radius" :min=1 :max=100 :step=1 @input='cesiumHeatmap.updateRadius(Heat_radius)' />
        </div>
        <template #footer>
            <div class="dialog-footer">
                <el-button @click="cesiumHeatmap.remove(); centerDialogVisible = false">取消加载热力图</el-button>
                <el-button type="primary" @click=" reloadHeatMap(); centerDialogVisible = false">
                    确定
                </el-button>
            </div>
        </template>
    ...
    <el-button plain @click="centerDialogVisible = true" id="HeatBtn">
        人口热力图设置
    </el-button>
    <el-dropdown @command="handleCommand">
        <el-button size="large" type="primary">
            功能列表<el-icon class="el-icon--right"><arrow-down /></el-icon>
        </el-button>
        <template #dropdown>
            <el-dropdown-menu>
                <el-dropdown-item command="fly">进行城市漫游</el-dropdown-item>
                <el-dropdown-item command="loadMask"
                    v-text="MASK.MaskVisible.value ? '取消加载边界' : '显示城市边界'"></el-dropdown-item>
                <el-dropdown-item command="loadStreet"
                    v-text="STREET.RoadVisible.value ? '取消加载路网' : '加载动态路网'"></el-dropdown-item>
            </el-dropdown-menu>
        </template>
    </el-dropdown>
    <router-link to="/city_Day"> <el-button id="btn" size="large" type="danger" text> 日间模式</el-button></router-link>
...
import { Geoserver_WFS } from "../utils/Geoserver";
import { addStreet } from "../utils/StreetMaterial";
import { ArrowDown } from '@element-plus/icons-vue';
import PolyLineMaterialProperty from '../utils/PolylineTrailMaterialProperty';
...
function createViewer(): void {
    ...
    //加载OSM底图
    let OSM = viewer.imageryLayers.addImageryProvider(new Cesium.UrlTemplateImageryProvider({
...
const MASK = {
    ...
    load: async function loadMask() {
        ...
        if (this.lineEntity)
            viewer.entities.add(this.lineEntity);
    ...
    remove: function removeMask() {
        if (this.mask)
            viewer.entities.remove(this.mask);
        if (this.lineEntity)
...
const STREET = {
    ...
    load: async function loadStreet() {
        await Geoserver_WFS('NewYork', 'NewYork_Road').then((res) => {
            ...
            if (res.data.features && res.data.features.length > 0) {
                ...
                (this.StreetPolyLine as any) = addStreet(viewer, features, '/public/PolyLine_material/Snipaste7.png')
            }
        })
    ...
    remove: function removeStreet() {
        if (this.StreetPolyLine)
            viewer.scene.primitives.remove(this.StreetPolyLine);
        this.RoadVisible.value = !this.RoadVisible.value;
    }
}
