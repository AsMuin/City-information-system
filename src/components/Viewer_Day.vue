  ...
  <el-dropdown @command="handleCommand">
    ...
    <template #dropdown>
      <el-dropdown-menu>
        ...
        <el-dropdown-item command="loadMask" v-text="MASK.MaskVisible.value ? '取消加载边界' : '显示城市边界'"></el-dropdown-item>
        <el-dropdown-item command="loadStreet"
          v-text="STREET.RoadVisible.value ? '取消加载路网' : '加载动态路网'"></el-dropdown-item>
      </el-dropdown-menu>
    </template>
  </el-dropdown>
  <router-link to="/city_Night"><el-button id="btn" size="large" type="danger" text> 夜间模式</el-button></router-link>
  ...
  load: async function loadStreet() {
    await Geoserver_WFS('NewYork', 'NewYork_Road').then((res) => {
      ...
      if (res.data.features && res.data.features.length > 0) {
        ...
        (this.StreetPolyLine as any) = addStreet(viewer, features, '/public/PolyLine_material/Snipaste5.png')
      }
    })
